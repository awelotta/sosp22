...
class Player(pg.sprite.Sprite):
    def __init__(self, grid_x, grid_y, stage, player):
        super().__init__()
        self.stage = stage
        self.player = player # whether you are player 1 or 2
        (width, height) = (stage.letter_width * 1, stage.letter_height * 1)
        self.image = pg.Surface((width, height))
        self.image.fill(PLAYER_COLOR if player == 0 else PLAYER2_COLOR)
    ...
    def move(self, text, stage, game):
        ...
        if d_pos is not None:
            self.wpm_calc.key()
            # collisions = game.move_collide(self.player, d_pos) # TODO handle collisions!!
        return d_pos
    def update(self, game):
        self.grid_pos = game.ppos[self.player]
        lw, lh = self.stage.letter_width, self.stage.letter_height
        self.rect.left = self.stage.rect.left + self.grid_pos[0] * lw
        self.rect.top = self.stage.rect.top + self.grid_pos[1] * lh
...
def render(player):
    ...
    screen.blit(*hud(player))
    pg.display.flip()

clock = pg.time.Clock()
n = Network()
player = int(n.p) # do I need a function?
...
players_group.add(p for p in players)
...
while True:
    ...
    for event in pg.event.get():
        ...
        # should I use text input? how should I organize the call??
            ...
            movement = me.move(event.text, stage, game) # hmm, is this the most general way to do things
            n.send(movement)
    update(game) # idt ths s effcient / a gd wy to cmprtmntlze
    render(me)
